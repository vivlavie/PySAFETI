def EventTree_OLF2007(pv,hole,lEvent):
    ...
    for e in lEvent:
        if ((pv in e.Key) and (hole in e.Hole)):
            epv,ehole,eweather = e.Key.split("\\")
            print("{:20s}{:10s} {:8.2e} {:8.2e} {:8.2e} - Leak Freq {:8.2e} ".format(epv,ehole,e.Frequency,e.PESD,e.PBDV,e.Frequency/e.PESD/e.PBDV))
            if basic_freq_set == False:
                ...
                if shET.cell(30,2).value != e.Frequency/e.PESD/e.PBDV:
            ...
            if ("EOBO" in ehole) or ("EOBN" in ehole):
                shET.cell(9,13).value = e.JetFire.Frequency*(1-P_FD_Fail) #Jet fire frequency = LeakFreq x PEO x PBO x PImgIgn / NumDir
            ...
            if "EOBX" in ehole:
                shET.cell(12,13).value = e.JetFire.Frequency*(1-P_FD_Fail)
            ...
            if ("EXBO" in ehole) or ("EXBN" in ehole):
                shET.cell(15,13).value = e.JetFire.Frequency*(1-P_FD_Fail)
            ...
            if "EXBX" in ehole:
                ...
                shET.cell(18,13).value = e.JetFire.Frequency*(1-P_FD_Fail)
                ...
                shET.cell(22,13).value = e.JetFire.Frequency/e.PESD/e.PBDV*(P_FD_Fail)                

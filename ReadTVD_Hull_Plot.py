...
for sfx in ['043-057','046-131']:
    tvExl = load_workbook(filename='h11_TVDischarge_'+sfx+'.xlsx')
    sh = tvExl['Sheet1']
    new_scn = False
    #iterte for all row
    #read total row
        ...
        if rv != None:
            if "Scenario" in rv:
                ...
                while t != None:
                    ...
                    tp = t
                    t = sh.cell(r,1).value            
                print("{:50s} | {:8.2f} sec | {:10.2f} [kg] ( {:4.1f} \%) | {:8.2f} [kg/s]".\
                    format(scn, tp, mm, mm/m0*100, rr))
                ...
                prj,part,vessel,eq,holesize = path.split("\\")
                # fn = slugify(path[20:])
                fn = vessel+"-"+eq+"-"+holesize                
